...
public class FullVisionScript : MonoBehaviour {
	...
	void Update () {
        ...
        Vector3 lowerLeft = Quaternion.AngleAxis(-hFOV, transform.up) * Quaternion.AngleAxis(-vFOV, transform.right) * camForward;
        Vector3 lowerRight = Quaternion.AngleAxis(hFOV, transform.up) * Quaternion.AngleAxis(-vFOV, transform.right) * camForward;
